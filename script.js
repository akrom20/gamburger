...
addCart.addEventListener('click', function () {
    ...
    for (let i = 0; i < bought.length; i++) {
        ...
        if (bought[i].amount > 0) {
           ...
           <div class="receipt__total">= ${bought[i].Summ}</div>
             <div class="total_kkal"> kkal= ${bought[i].KkalSum}</div>
           </div>
 ...
 if ( totalPrice > 0) {
    receiptWindowOut.innerHTML = `${total}<div class="receipt__totalPrice">total price: ${totalPrice + dastavka} \nyetkazib berish narxi: ${dastavka} sum `
 }
...
const reload = document.querySelector('.receipt__window-btn')
